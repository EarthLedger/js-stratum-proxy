...
class MinerSocket extends JsonTcpSocket {
	...
	async onData(data) {
		...
		if (method === 'mining.authorize' || method === 'login') {
			...
			this.pool.on('authenticated', e => this.log('Authenticated'))
			this.pool.on('shareAccepted', e => this.log('Share Accepted'))
			this.pool.on('shareRejected', e => this.log('Share Rejected'))
			this.pool.on('notify', e => this.log('New Job'));
	...
	log(message) {
		this.emit(
			'log',
			`[${moment().format()}][${this.minerName}][${this.address || ''}] ${message}`
		);
	}
}
